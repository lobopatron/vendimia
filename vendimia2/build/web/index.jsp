    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function arr() {
                ...
                if (items[art - 1][4] == "0") {
                    alert("No hay modelos en existencia");
                } else {
        ...
        <td>" + items[art - 1][2] + "</td>\n\
<td><input type=\"text\" id=\"numArt\" name=\"numArt\" style=\"width:2em;\" onkeypress=\"numero(this);\" class=\"input\" required> \n\
/" + items[art - 1][4] + "</td>\n\
...
<input type=\"button\" id=\"cancel\" name=\"agregarArt\" value=\"\" style=\"display: none;\" onclick=\"cancelar(" + items[art - 1][0] + ");\" >";
            ...
            function comparar(nodo) {
                ...
                var existe = parseFloat(nodosEnTr[3].textContent);
                if (numero <= existe && numero > 0) {
                    nodosEnTr[5].textContent = numero * nodosEnTr[4].textContent;
                    enganch();
                }
                if (numero >= existe) {
                ...
                if (numero < 1) {
                    alert("Selecciona por lo menos 1");
                    nodoTr.getElementsByTagName('input')[0].value = "";
                    nodoTr.getElementsByTagName('input')[0].focus();
                }
    ...
    <body>
        ...
        <%
            if (session.getAttribute("noVenta") != null) {
        %>
        <div class="avisoMal">
            <h3>
                <%
                    out.print(session.getAttribute("noVenta"));
                %>
            </h3>
        </div>
        <%
            }
            if (session.getAttribute("saveVenta") != null) {
        %>
        <div class="avisoBien">
            <h3>
                <%
                    out.print(session.getAttribute("saveVenta"));
                %>
            </h3>
        </div>
        <%
            }
            session.setAttribute("saveVenta", "");
            session.setAttribute("noVenta", "");
        %>
        <section id="cont1">
            <div id="newvent">
            ...
            <section id="cont2" style="display: none;">
                ...
                <div id="registro">
                    <div class="renglon_venta">
                        ...
                        <div class="label_input">
                            <input type="text" id="rfccliente" name="cliVenta" class="input" placeholder="RFC" disabled>
                        </div>
                    </div>
                    <div class="renglon_venta">
                        <div class="label">
                    ...
                    <div id="totales" style="display: none;">
                        <div class="precios">
                            <label> Enganche:</label> <input type="text" id="enganche" value="0" name="enganche" class="input" placeholder="Enganche" disabled>
                        </div>
                        <div class="precios">
                            <label>Bonificacion Enganche:</label> <input type="text" value="0" id="boni" name="bonificacion" class="input" disabled placeholder="Bonificacion de Enganche">
                        </div>
                        <div class="precios">
                            <label>Total: </label><input type="text" id="total" value="0" name="total" class="input" placeholder="Total" disabled>
                        </div>
                    </div>
